...
namespace vnaisoft.common.Controllers
{
    ...
    [ApiController]
    [Route("[controller]")]
    public class ChuKySoController : Controller
    {
        public MongoDBContext _context;
        public HttpClient client;

        public AppSettings _appsetting;
        public ChuKySoController(IOptions<AppSettings> appsetting, MongoDBContext context)
        {
        ...
        [HttpPost("ky_so")]
        public async Task<IActionResult> ky_so()
        {
            var link = "https://mobifonehsm.vn/";
            var error = "";
            var success = "";
            var values = new Dictionary<string, string> { { "username", "" }, { "password", "" } };
            var stringContentLogin = new StringContent(JsonConvert.SerializeObject(values), Encoding.UTF8, "application/json");
            var responseLogin = client.PostAsync(link + "Pdf/api/Account/Login", stringContentLogin).Result;
            string jsonDataLogin = responseLogin.Content.ReadAsStringAsync().Result;

            JToken tokenJ = JObject.Parse(jsonDataLogin);
            var last = tokenJ.Last().Last();
            var token = last.SelectToken("token").ToString();
        ...
        [HttpPost("uploadpdf")]
        [Consumes("multipart/form-data")] // Chỉ định kiểu dữ liệu tiêu thụ
        public async Task<IActionResult> UploadPdfFormData([FromForm] IFormFile pdfFile)
        {
            if (pdfFile == null || pdfFile.Length == 0)
            {
                return BadRequest("Không có file nào được gửi hoặc file rỗng.");
            }

            if (pdfFile.ContentType != "application/pdf")
            {
                // Kiểm tra Content-Type của file
                return BadRequest("File gửi lên không phải là định dạng PDF.");
            }

            try
            {
                // Tạo một thư mục để lưu trữ các file upload
                ...
                return Ok(new { Message = "File PDF đã được upload thành công!", FileName = uniqueFileName, FilePath = filePath });
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult> upload_file()
        {
            ...
            if (item != null)
            {
                ...
                var path = Path.Combine(currentpath, "file_upload", "chat_upload", id);
