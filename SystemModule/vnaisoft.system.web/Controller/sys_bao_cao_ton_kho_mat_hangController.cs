    ...
    public partial class bao_cao_ton_kho_mat_hangController : BaseAuthenticationController
    {
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                var queryTable = repo._common_repo._context.sys_ton_kho_mat_hang_col.AsQueryable()
        ...
        .ToList());
                dataList.ForEach(q =>
                {
                    q.ten_don_vi_tinh = repo._context.sys_don_vi_tinh_col.AsQueryable().Where(d => d.id == q.id_don_vi_tinh).Select(d => d.ten).SingleOrDefault();
