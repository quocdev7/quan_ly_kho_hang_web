    ...
    public partial class bao_cao_nhap_khoController : BaseAuthenticationController
    {
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                //var lst_pn = repo._context.sys_phieu_nhap_khos.AsQueryable().Where(q=>q.status_del==1).Select(q => q.id).ToList();

                var querytable = repo._context.sys_phieu_nhap_kho_chi_tiet_col.AsQueryable()
        ...
        .ToList());
                dataList.ForEach(q =>
                {
                    //var mat_hang = repo._context.sys_mat_hang_col.AsQueryable().Where(d => d.id == q.id_mat_hang).FirstOrDefault();
