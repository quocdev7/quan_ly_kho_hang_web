...
namespace vnaisoft.system.web.Controller
{
    public partial class sys_don_hang_banController : BaseAuthenticationController
    {
        ...
        //public async Task<IActionResult> get_cau_hinh_he_thong_gia_ban([FromBody] JObject json)
        //{
        //    var result = repo._context.erp_cau_hinh_he_thongs.AsQueryable()
        //        .Where(d => d.code == "gia_ban_gom_thue")
        //        .Where(d => d.status_del == 1).SingleOrDefault();
        //    return Json(result);
        //}
        ...
        //    id_kho = repo._context.Users.AsQueryable().Where(q => q.Id == getUserId()).Select(q => q.id_kho_xuat).SingleOrDefault();


        //    return Json(id_kho);
        //}
        //public IActionResult get_list_nhan_vien()
        //{
        //    var queryTable = repo._context.sys_don_hang_bans.AsQueryable().Where(d => d.status_del == 1);

        //    var result = repo.FindAll(queryTable)
        //         .Select(d => new
        //         {
        //             id = d.db.nguoi_cap_nhat,
        //             name = d.ten_nguoi_cap_nhat,
        //         }).Distinct().ToList();
        //    return Json(result);
        //}
        ...
        //    var template = repo._context.erp_mau_ins.AsQueryable().Where(t => t.id_loai == "sys_don_hang_ban").FirstOrDefault();
        ...
        //            List<sys_don_hang_ban_model> list_import = new List<sys_don_hang_ban_model>();
        //            for (int ct = 0; ct < list_row.Count(); ct++)
        //            {
        //                var fileImport = list_row[ct].list_cell.ToList();


        //                var model = new sys_don_hang_ban_model();
        ...
        //                var tinh_trang_don_hang = (fileImport[7].value.ToString() ?? "").Trim();
        //                var loai_don_hang = (fileImport[8].value.ToString() ?? "").Trim();
        //                //var ten_don_hang = (fileImport[3].value.ToString() ?? "").Trim();
        //                var ghi_chu = (fileImport[9].value.ToString() ?? "").Trim();
        //                var ma_doi_tuong = (fileImport[10].value.ToString() ?? "").Trim();
        //                var phuong_thuc_thanh_toan = (fileImport[11].value.ToString() ?? "").Trim();
        //                var so_tai_khoan_ngan_hang = (fileImport[12].value.ToString() ?? "").Trim();
        //                var hinh_thuc_van_chuyen = (fileImport[13].value.ToString() ?? "").Trim();
        //                var ngay_dat_hang = (fileImport[14].value.ToString() ?? "").Trim();
        //                var chi_phi_van_chuyen = (fileImport[15].value.ToString() ?? "").Trim();
        //                var thue_van_chuyen = (fileImport[16].value.ToString() ?? "").Trim();
        //                var ma_don_vi_van_chuyen = (fileImport[17].value.ToString() ?? "").Trim();
        //                var ma_don_van_chuyen = (fileImport[18].value.ToString() ?? "").Trim();
        //                var so_ngay_du_kien_giao = (fileImport[19].value.ToString() ?? "").Trim();
        //                var ngay_den_han_thanh_toan = (fileImport[20].value.ToString() ?? "").Trim();
        //                var so_dien_thoai_nguoi_nhan = (fileImport[21].value.ToString() ?? "").Trim();
        //                var dia_diem_giao_hang = (fileImport[22].value.ToString() ?? "").Trim();
        //                var is_da_thu = (fileImport[23].value.ToString() ?? "").Trim();
        //                var is_da_xuat = (fileImport[24].value.ToString() ?? "").Trim();
        //                var is_hang_tang = (fileImport[25].value.ToString() ?? "").Trim();

        //                model.db.tinh_trang_don_hang = tinh_trang_don_hang;
        //                model.db.loai_giao_dich = int.Parse(loai_don_hang);
        ...
        //                var doi_tuong = repo._context.erp_khach_hang_nha_cung_caps.AsQueryable().Where(d => d.id == ma_doi_tuong).SingleOrDefault();
        //                if (model.db.id_doi_tuong == "DTTD" || model.db.id_doi_tuong == null)
        //                {
        //                    model.db.hinh_thuc_doi_tuong = doi_tuong.hinh_thuc;
        //                    model.check_doi_tuong = 1;
        ...
        //                {
        //                    var ngan_hang = repo._context.erp_tai_khoan_ngan_hangs.AsQueryable().Where(q => q.so_tai_khoan == so_tai_khoan_ngan_hang).Where(q => q.status_del == 1).SingleOrDefault();
        //                    if (ngan_hang != null)
        //                    {
        //                        model.db.id_tai_khoan_ngan_hang = ngan_hang.id;
        //                        model.db.ma_ngan_hang = ngan_hang.ma_ngan_hang;
        //                        model.db.so_tai_khoan = ngan_hang.so_tai_khoan;
        //                    }
        //                }
        //                model.db.hinh_thuc_van_chuyen = int.Parse(hinh_thuc_van_chuyen);
        ...
        //                //{
        //                //    model.db.ngay_du_kien_giao_hang = day_now.AddDays(int.Parse(so_ngay_du_kien_giao));
        //                //}
        ...
        //                    var vat = Constant.list_vat.Where(q => q.id.Trim().ToLower() == model.db.vat_van_chuyen).Select(q => q.value).SingleOrDefault();
        //                    model.db.tien_vat_van_chuyen = (model.db.tien_van_chuyen ?? 0) * vat / 100;
        //                    model.db.id_don_vi_van_chuyen = ma_don_vi_van_chuyen;
        //                    model.db.ma_don_van_chuyen = ma_don_van_chuyen;
        //                    model.db.so_ngay_du_kien = int.Parse(so_ngay_du_kien_giao);
        //                    model.db.ngay_du_kien_giao_hang = model.db.ngay_dat_hang.Value.AddDays((double)model.db.so_ngay_du_kien);
        //                    model.db.ngay_den_han_thanh_toan = DateTime.Parse(ngay_den_han_thanh_toan);
        //                    if (model.db.id_doi_tuong == "DTTD")
        //                    {
        //                        model.db.so_dien_thoai_nguoi_nhan = so_dien_thoai_nguoi_nhan;
        //                        model.db.dia_chi_giao_hang = dia_diem_giao_hang;
        //                    }
        //                    else
        //                    {
        //                        model.db.so_dien_thoai_nguoi_nhan = doi_tuong.dien_thoai;
        //                        model.db.dia_chi_giao_hang = doi_tuong.dia_chi;
        //                    }
        //                }
        //                model.db.is_thu_du = int.Parse(is_da_thu) == 1 ? true : int.Parse(is_da_thu) == 2 ? false : null;
        //                model.db.is_xuat_du = int.Parse(is_da_xuat) == 1 ? true : int.Parse(is_da_xuat) == 2 ? false : null;
        //                model.db.is_hang_tang = int.Parse(is_hang_tang) == 1 ? true : int.Parse(is_hang_tang) == 2 ? false : null;
        //                //user import
        ...
        //                    dh.list_mat_hang = list_import.Where(q => q.db.ma == item).Select(d => new sys_don_hang_ban_mat_hang_model
        //                    {
        ...
        //                        vat = repo._context.erp_mat_hangs.AsQueryable().Where(q => q.ma == d.ma_mat_hang).Select(q => q.vat).SingleOrDefault(),
        //                    }).ToList();
        ...
        //                    dh.db.id_kho_xuat_ban_le = repo._context.Users.AsQueryable().Where(d => d.Id == getUserId()).Select(d => d.id_kho_xuat).SingleOrDefault();
        //                    dh.db.id = dh.db.ma;
        //                    dh.db.kieu_ban = 1;
        ...
        //            db_log.error = error;
        //            db_log.controller = "sys_don_hang_ban";
        ...
        //}
        //[HttpPost]
        //public async Task<IActionResult> ImportFromExcelChiTiet()
        //{
        //    var error = "";
        //    IFormFile file = Request.Form.Files[0];

        //    var name = "DonHangBanBuon_ChiTiet";
        ...
        //                var model = new sys_don_hang_ban_model();
        //                var stt = (fileImport[0].value.ToString() ?? "").Trim();
        //                var nhan_hieu = (fileImport[1].value.ToString() ?? "").Trim();
        //                var xuat_xu = (fileImport[2].value.ToString() ?? "").Trim();
        //                var ma_loai_mat_hang = (fileImport[3].value.ToString() ?? "").Trim();
        //                var ma_don_vi_tinh = (fileImport[4].value.ToString() ?? "").Trim();
        //                var ma_vach = (fileImport[5].value.ToString() ?? "").Trim();
        //                var ma_mat_hang = (fileImport[6].value.ToString() ?? "").Trim();
        //                var ten_mat_hang = (fileImport[7].value.ToString() ?? "").Trim();
        //                var gia_ban_le = (fileImport[8].value.ToString() ?? "").Trim();
        //                var gia_ban_si = (fileImport[9].value.ToString() ?? "").Trim();
        //                var thuoc_tinh = (fileImport[10].value.ToString() ?? "").Trim();
        //                var vat = (fileImport[11].value.ToString() ?? "").Trim();
        //                var ghi_chu = (fileImport[12].value.ToString() ?? "").Trim();
        ...
        //    var error = "";
        //    IFormFile file = Request.Form.Files[0];
        //    var model_master = JsonConvert.DeserializeObject<sys_don_hang_ban_model>(Request.Form["model"]);
        //    var rs = "";
        ...
        //                var model = new sys_don_hang_ban_mat_hang_model();
        ...
        //                    model.db.chiet_khau = chiet_khau == "" ? mat_hang.ty_le_chiet_khau : decimal.Parse(chiet_khau);
        //                    model.db.don_gia = don_gia == "" ? mat_hang.gia_ban_si : decimal.Parse(don_gia);
        ...
        //    string Files = newPath + "\\sys_don_hang_ban.xlsx";
        //    byte[] fileBytes = System.IO.File.ReadAllBytes(Files);
        //    System.IO.File.WriteAllBytes(Files, fileBytes);
        //    MemoryStream ms = new MemoryStream(fileBytes);
        //    return File(fileBytes, System.Net.Mime.MediaTypeNames.Application.Octet, "sys_don_hang_ban.xlsx");
        //}
        //[AllowAnonymous]
        ...
        //    string Files = newPath + "\\erp_mat_hang_theo_don_hang_ban.xlsx";
        //    byte[] fileBytes = System.IO.File.ReadAllBytes(Files);
        //    System.IO.File.WriteAllBytes(Files, fileBytes);
        //    MemoryStream ms = new MemoryStream(fileBytes);
        ...
        //public async Task<IActionResult> cap_nhap([FromBody] JObject json)
        //{
        //    var id = json.GetValue("id").ToString();
        //    var tinh_trang_don_hang = json.GetValue("tinh_trang_don_hang").ToString();
        //    repo.cap_nhap(id, getUserId(), tinh_trang_don_hang);
        //    return Json("");
        //}
        ...
        //[HttpPost]
        //public async Task<IActionResult> ImportFromExcelDetail()
        //{
        //    var error = "";
        //    IFormFile file = Request.Form.Files[0];
        //    var model_master = JsonConvert.DeserializeObject<sys_don_hang_ban_model>(Request.Form["model"]);
        ...
        //                var ma_mat_hang = (fileImport[1].value.ToString() ?? "").Trim();
        //                var so_luong = (fileImport[2].value.ToString() ?? "").Trim();
        //                var chiet_khau = (fileImport[3].value.ToString() ?? "").Trim();
        //                var ghi_chu = (fileImport[4].value.ToString() ?? "").Trim();

        //                var mat_hang = repo._context.erp_mat_hangs.AsQueryable().Where(q => q.ma == ma_mat_hang).FirstOrDefault();

        //                model.db.id_mat_hang = mat_hang == null ? null : mat_hang.id;
        //                model.db.id_don_vi_tinh = mat_hang == null ? null : mat_hang.id_don_vi_tinh;
        //                model.db.vat = mat_hang == null ? null : mat_hang.vat;
        //                model.db.so_luong = decimal.Parse(so_luong);
        //                model.db.don_gia = mat_hang == null ? null : mat_hang.gia_ban_si;
        //                model.db.chiet_khau = decimal.Parse(chiet_khau);
        //                model.db.ghi_chu = ghi_chu;
        //                model.id_mat_hang = mat_hang == null ? null : mat_hang.id;
        //                model.ten_mat_hang = mat_hang == null ? null : mat_hang.ten;
        //                model.ma_mat_hang = mat_hang == null ? null : mat_hang.ma;

        //                model.ten_don_vi_tinh = mat_hang == null ? null : repo._context.erp_mat_hangs.AsQueryable().Where(q => q.id == model.db.id_don_vi_tinh).Select(q => q.ten).FirstOrDefault();
        //                //user import
        //                error = CheckErrorImportDetail(model, ct + 1, error);
        //                if (!string.IsNullOrEmpty(error))
        //                {
        ...
        //                else
        //                {
        //                    model.ten_don_vi_tinh = repo._context.erp_don_vi_tinhs.AsQueryable().Where(q => q.id == model.db.id_don_vi_tinh).Select(q => q.ten).SingleOrDefault();
        //                    model_master.list_mat_hang.Add(model);
        //                }
        ...
        //public async Task<IActionResult> get_list_mat_hang([FromBody] JObject json)
        //{
        //    var id_don_hang = json.GetValue("id_don_hang").ToString();
        //    var queryTableDetail = repo._context.erp_phieu_xuat_kho_chi_tiets.AsQueryable().Where(q => q.is_vat != true);
        //    var id_phieu_xuat_kho = repo._context.erp_phieu_xuat_khos.AsQueryable().Where(q => q.id_don_hang_ban == id_don_hang).Select(q => q.id).SingleOrDefault();
        //    var list_mat_hang = repoXK.FindAllDetail(queryTableDetail).Where(q => q.db.id_phieu_xuat_kho == id_phieu_xuat_kho).ToList();
        //    return Json(list_mat_hang);
        //}

        //public async Task<IActionResult> get_list_don_hang_ban([FromBody] JObject json)
        //{
        //    var list_don_hang_ban = new List<sys_don_hang_ban_model>();

        //    var queryTable = repo._context.sys_don_hang_bans.AsQueryable().Where(q => q.status_del == 1);
        //    list_don_hang_ban = repo.FindAll(queryTable).Where(q => q.db.status_del == 1).ToList();
        //    return Json(list_don_hang_ban);
        //}
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                var is_thu_du = int.Parse(dictionary["is_thu_du"]);
                ...
                //if (is_xuat_du == 1)
                //{
                //    queryTable = queryTable.Where(q => q.is_xuat_du == true);
                //}
                //if (is_xuat_du == 0)
                //{
                //    queryTable = queryTable.Where(q => q.is_xuat_du != true);
                ...
                //if (is_thu_du == 1)
                //{
                //    queryTable = queryTable.Where(q => q.is_thu_du == true);
                //}
                //if (is_thu_du == 0)
                //{
                //    queryTable = queryTable.Where(q => q.is_thu_du != true);
    ...
    //    }
        ...
        //                var stt = (fileImport[0].value.ToString() ?? "").Trim();
        //                var ten_don_vi = (fileImport[1].value.ToString() ?? "").Trim();
        //                var ghi_chu = "";
        //                if (fileImport.Count() == 4)
        //                    ghi_chu = (fileImport[2].value.ToString() ?? "").Trim();
        //                model.db.ma = ten_don_vi;
        //                model.db.ghi_chu = ghi_chu;
