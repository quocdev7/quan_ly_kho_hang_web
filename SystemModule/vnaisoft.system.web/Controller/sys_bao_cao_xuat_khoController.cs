...
//                //var lst_px = repo._context.erp_phieu_xuat_khos.AsQueryable().Where(q => q.status_del == 1).Select(q => q.id).ToList();

//                var querytable = repo._context.erp_phieu_xuat_kho_chi_tiets.AsQueryable()
...
//                    var doi_tuong = repo._context.erp_phieu_xuat_khos.AsQueryable().Where(d => d.id == q.id_phieu_xuat_kho).SingleOrDefault();
...
//                    var id_don_vi_tinh = repo._context.erp_mat_hangs.AsQueryable().Where(d => d.id == q.ma_mat_hang).Select(d => d.id_don_vi_tinh).SingleOrDefault();
//                    q.ten_don_vi_tinh = repo._context.erp_don_vi_tinhs.AsQueryable().Where(d => d.id == id_don_vi_tinh).Select(d => d.ten).SingleOrDefault();
