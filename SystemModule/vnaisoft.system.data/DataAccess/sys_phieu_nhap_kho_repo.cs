...
namespace vnaisoft.system.data.DataAccess
{
    public class sys_phieu_nhap_kho_repo
    {
        ...
        public async Task<sys_phieu_nhap_kho_model> getElementById(string id)
        {
            ...
            //loai == 1 nhap
            //loai == 2 xuat
            var ma_loai_xuat = _context.sys_loai_nhap_xuat_col.AsQueryable().Where(q => q.id == obj.db.id_loai_nhap).Select(q => q.ma).SingleOrDefault();
            if (obj.db.nguon == 2)
            {
        ...
        public async Task removeOldPhieuAsync(string id_phieu)
        {
            ...
            {
                await _common_trigger_tonkho_repo.updateNhapTonKhoAsync(-listOld[i].so_luong, -listOld[i].gia_tri, listOld[i].id_mat_hang, listOld[i].id_don_vi_tinh, old_db.ngay_nhap);
            }
        }
        public async Task<int> upset_detail(sys_phieu_nhap_kho_model model)
        {
            ...
            for (int i = 0; i < model.list_mat_hang.Count(); i++)
            {
                ...
                await _common_trigger_tonkho_repo.updateNhapTonKhoAsync(data.db.so_luong, 0, data.db.id_mat_hang, data.db.id_don_vi_tinh, model.db.ngay_nhap);
            }
            return 1;
        }
