...
namespace vnaisoft.system.data.DataAccess
{
    public class bao_cao_nhap_kho_repo
    {
        ...
        public XSSFWorkbook exportExcelRepo(XSSFWorkbook workbook, string search, DateTime tu_ngay, DateTime den_ngay, string id_kho, string id_loai_mat_hang, AppSettings _appsetting, string filename)
        {
            ...
            dataList.ForEach(q =>
            {
                q.ten_loai_nhap = _context.sys_loai_nhap_xuat_col.AsQueryable().Where(d => d.ma == q.ma_loai_nhap).Select(d => d.ten).FirstOrDefault();
            ...
            header = new string[] {
                       "STT (No.)","Loại nhập","Mã phiếu nhập kho","Ngày nhập kho","Mã Loại mặt hàng","Tên loại mặt hàng","Mã mặt hàng","Tên mặt hàng"
                       ,"Số lượng","Đơn vị tính"
                   };

            listKey = new string[]
            {
                       "ten_loai_nhap","ma_phieu_nhap_kho","ngay_nhap_kho","ma_loai_mat_hang","ten_loai_mat_hang","StrExcel_ma_mat_hang","ten_mat_hang"
                       ,"Num_so_luong","ten_don_vi_tinh"
            };
