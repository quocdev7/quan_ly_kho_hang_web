...
//            var querytable = _context.erp_phieu_nhap_kho_chi_tiets.AsQueryable()
...
//            dataList.ForEach(q =>
//            {
//                q.ten_loai_nhap = _context.erp_loai_nhap_xuats.AsQueryable().Where(d => d.ma == q.ma_loai_nhap).Select(d => d.ten).SingleOrDefault();
...
//                q.ten_mat_hang = _context.erp_mat_hangs.AsQueryable().Where(d => d.id == q.ma_mat_hang).Select(d => d.ten).FirstOrDefault();
...
//                var doi_tuong = _context.erp_phieu_nhap_khos.AsQueryable().Where(d => d.id == q.id_phieu_nhap_kho).SingleOrDefault();
...
//        public IQueryable<bao_cao_nhap_kho_model> FindAll(IQueryable<erp_phieu_nhap_kho_chi_tiet_db> query)
//        {

//            var result = (from d in query.OrderByDescending(q => q.ngay_nhap)
//                          join dvt in _context.erp_don_vi_tinhs.AsQueryable() on d.id_don_vi_tinh equals dvt.id into gdvt
...
//                              //ten_doi_tuong = pnk.ten_doi_tuong,
//                              //ma_so_thue = pnk.ma_so_thue,
//                              //dien_thoai = pnk.dien_thoai,
//                              //email = pnk.email,
//                              //dia_chi_doi_tuong = pnk.dia_chi_doi_tuong,
//                          });
//            return result;
