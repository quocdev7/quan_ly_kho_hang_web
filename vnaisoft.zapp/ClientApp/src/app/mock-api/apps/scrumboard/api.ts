    ...
    registerHandlers(): void
    {
        ...
        this._fuseMockApiService
            ...
            .reply(({request}) => {
                ...
                board.lists = this._lists.filter(item => item.in_contract === id).sort((a, b) => a.position - b.position);

                // Grab all cards that belong to this board and attach labels to them
                let cards = this._cards.filter(item => item.in_contract === id);
                ...
                board.lists.forEach((list, index, array) => {
                    array[index].cards = cards.filter(item => item.in_contract === id && item.listId === list.id).sort((a, b) => a.position - b.position);
                });

                // Attach the board labels
                board.labels = this._labels.filter(item => item.in_contract === id);
        ...
        this._fuseMockApiService
            ...
            .reply(({request}) => {
                ...
                this._cards.forEach((card) => {
                    ...
                    card.position = cards.findIndex(item => item.id === card.id && item.listId === card.listId && item.in_contract === card.in_contract);
                });
        ...
        this._fuseMockApiService
            ...
            .reply(({request}) => {
                ...
                // Get the board id
                const in_contract = request.params.get('in_contract');

                // Filter the labels
                const labels = this._labels.filter(item => item.in_contract === in_contract);
