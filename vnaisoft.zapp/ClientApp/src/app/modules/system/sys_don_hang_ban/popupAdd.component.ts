...
export class sys_don_hang_ban_popUpAddComponent extends BasePopUpAddTypeComponent<sys_don_hang_ban_model> {
    ...
    constructor(public dialogRef: MatDialogRef<sys_don_hang_ban_popUpAddComponent>,
        ...
        if (this.actionEnum == 1) {
            ...
            this.record.cau_hinh_gia_ban = data.cau_hinh_gia_ban;
    ...
    checkKho($event) {
        if ($event.checked == true) {
            if (this.record.db.id_kho_xuat_ban_le == null || this.record.db.id_kho_xuat_ban_le == "") {
                Swal.fire("Bạn chưa được thiết lập kho xuất bán. Vui lòng liên hệ người quản trị để được thiết lập.", "", "warning").then(() => {
                    this.record.db.is_sinh_tu_dong = false
                    $event.checked = false
                });
            }
    ...
    openDialogChooseMatHang(): void {
        ...
        //     Swal.fire(this._translocoService.translate('erp.phai_chon_kieu_ban'), '', 'warning')
        // } else {
    ...
    load_ngay_du_kien() {
        ...
        this.record.db.ngay_du_kien_giao_hang = ngay_du_kien_giao_hang;
    }
    load_so_ngay_du_kien() {
        var date = new Date(this.record.db.ngay_du_kien_giao_hang);
        this.record.db.so_ngay_du_kien = date.getDate();
    }
